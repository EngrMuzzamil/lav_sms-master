<script>
    ...
    function getCurrentStudents(){
        ...
        var url = '{{url('/promotion/getCurrentStudents')}}';
        var stds = $("#student_ids");
        $.ajax({
            dataType: 'json',
            method: 'GET',
            data:{'semester_id':semester_id,'session_id':session_id},
            url: url,
            success: function (resp) {
                ...
                $.each(resp, function (i, data) {
                    ...
                    stds.append($('<option>', {
                        selected:'selected',
                        value: data.id,
                        text: data.student_name+'-'+data.roll_no
                    }));
                });

                $("#loader").html('Students');
            }
        })
    }

    function getPromotedStudents(){
        $("#loader1").html("loading..........")
        var semester_id = $("#s_semester_id").val();
        var session_id = $("#s_session_id").val();
        if(session_id == '' || semester_id == '' ){
            alert("Please select session/semester");
            return false;
        }
        var url = '{{url('/promotion/getPromotedStudents')}}';
        var stds = $("#promoted");
        stds.html("<tr><td colspan='3' class='text-center'> <h4>Loading</h4></td></tr>")
        $.ajax({
            dataType: 'json',
            method: 'GET',
            data:{'semester_id':semester_id,'session_id':session_id},
